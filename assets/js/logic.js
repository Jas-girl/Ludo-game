...
var step_sound = new Audio('./assets/audio/assets_audio_step.mp3');
var dead_sound = new Audio('./assets/audio/assets_audio_dead.mp3');
var inout_sound = new Audio('./assets/audio/assets_audio_inout.mp3');
var dice_sound = new Audio('./assets/audio/assets_audio_dice.mp3');
var winner_sound = new Audio('./assets/audio/assets_audio_winner.mp3');
...
];//turns are managed in this order

var turn_oder = [];
var current_turn_index = 0;//it is updated after each move
...
$("#dice").click(function () {
    ...
    dice_sound.play();
    //conditions the dice value come and activate the players
    if (getActivePlayers(current_turn.players[0].home) == 0 && dice_value != 6) {
        $('.msg').text(current_turn.group + " : no player is active, shifting turn to next player");
        hold(hold_time).then(() => {
            updateTurn(++current_turn_index);
            activateDice();
        });
        ...
        $('.msg').text(current_turn.group + " : you can't use this dice value, so shifting turn to next player");
        hold(hold_time).then(() => {
            updateTurn(++current_turn_index);
            activateDice();
        });
    } else if (getActivePlayers(current_turn.players[0].home) == 1 && dice_value < 6) {
        $('.msg').text(current_turn.group + " : you only have 1 active player so moving automatically");
        ...
        $('.msg').text(current_turn.group + " : you got 6 but , you don't have any active players and only 1 player in home so making it active");
        hold(hold_time).then(() => {
            for (i = 0; i < 4; i++) {
...
function movePlayer(M_Player, M_Step, M_Road) {
    ...
    if (M_Step == 0) {
       $('.msg').text(M_Player.color + ' : Dice value is 0');
        return 0;
    }

    if (M_Step == 6 && M_Player.status == 'home') {
        M_Player.status = "active";
        $('.msg').text(M_Player.color + ' : Player is active now');
    ...
    if (M_Step != 0 && M_Player.status == 'home') {
        $('.msg').text(M_Player.color + ' : player is not active');
